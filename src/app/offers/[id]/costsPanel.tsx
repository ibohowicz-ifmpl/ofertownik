...
export default function CostsPanel({ offerId }: { offerId: string }) {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) {
          setCosts(padded);
          setBaseline(padded);
          setValueText(padded.map((c) => (c.valueNet == null ? "" : formatMoney(c.valueNet))));
        }
      } catch {
        // zostaw puste 5
      }
  ...
  const anyDirty = costs.length !== baseline.length || costs.some((_, i) => isRowDirty(i));

  const sum = useMemo(
    () =>
      costs.reduce((acc, c) => acc + (Number.isFinite(Number(c.valueNet)) ? Number(c.valueNet) : 0), 0),
    [costs]
  );
      ...
      <div className="rounded-lg border border-gray-200 bg-white shadow-sm p-3">
          ...
          {Array.from({ length: 5 }).map((_, i) => {
            ...
            return (
              <div key={i} className="flex items-center gap-2 w-full">
                <input
                  className={`border rounded px-2 py-1 grow min-w-[16rem] ${dirty ? "ring-1 ring-yellow-400 bg-yellow-50" : ""}`}
        ...
        <div className="mt-3 flex items-center justify-end">
          ...
          <button
            ...
            title={anyDirty ? "Masz niezapisane zmiany kosztów" : "Brak zmian do zapisania"}
          >
            Zapisz koszty
          </button>
        </div>
