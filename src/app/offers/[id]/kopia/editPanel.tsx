...
export default function EditPanel({
  ...
  async function deleteOffer() {
    try {
      ...
      setSaveMsg({ text: "Usunięto ofertę.", type: "success" });
      setTimeout(() => {
        window.location.href = "/offers?msg=deleted";
      }, 300);
    } catch (e: any) {
      setSaveMsg({ text: e?.message || "Nie udało się usunąć oferty.", type: "error" });
      setTimeout(() => setSaveMsg(null), 2500);
    }
  }
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <label className="grid gap-1">
            ...
            <input
              ...
              onBlur={(e) => {
                const n = Number((e.currentTarget.value || "").replace(SPACE_RE, "").replace(",", "."));
                if (Number.isFinite(n)) setFields((prev) => ({ ...prev, valueNet: formatMoney(n) }));
              }}
              onFocus={(e) => {
                const n = Number((fields.valueNet || "").replace(SPACE_RE, "").replace(",", "."));
                e.currentTarget.value = Number.isFinite(n) ? String(n).replace(".", ",") : fields.valueNet || "";
              }}
